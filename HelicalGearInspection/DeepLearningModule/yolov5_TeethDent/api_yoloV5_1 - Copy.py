    ...
    def score_frame(self, frame): 
        ...
        for *xyxy, conf, cls in det:
            ...
            if int(xyxy[0])<=40 or int(xyxy[0])>=500:#handliong cornes dent(x cordinate)
                continue
            elif lab =='Flank_unclean or Rust' and int(xyxy[1])>=500 and center_y<50: #y-coordinate  
                continue
            elif lab =='Flank_unclean or Rust' and  int(xyxy[1])<=140 and center_y<50:
                continue
            elif lab =='Flank_unclean or Rust' and cf<0.60:#int(xyxy[1])<=140: #
                continue
...
def pers_transf(img):
    ...
    pts1 = np.float32([[0, 440], [1180, 0], [0, 1750], [1200, 1280]])
    #pts1 = np.float32([[0,300],[1000,0],[0,1850],[1200,1500]])
...
def predictions():
    try:
        ...
        frame = pers_transf(openCvImage)  # correction perspective
        ...
        return jsonify(defImage=base64Image, serialized_Defects=["Ok"])
        #return ex
